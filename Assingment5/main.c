...
void lcd_write_address(char address){
 ...
 {
	char OutString[100];
   // sprintf(OutString, "%d%d:%d%d:%d%d ", hours_tenth,hours,min_tenth, minutes,sec_tenth, seconds);
	...
	if(hours <10){
		sprintf(OutString, "0%d:", hours);

	}
	else
	{
		sprintf(OutString, "%d", hours);

	}
	lcd_word(OutString);
	if(minutes < 10){
		sprintf(OutString, "0%d:", minutes);
	}
	else
	{
		sprintf(OutString, "%d", minutes);
	}
	lcd_word(OutString);
	if(seconds < 10){
		sprintf(OutString, "0%d", seconds);

	}
	else
	{
		sprintf(OutString, "%d", seconds);
